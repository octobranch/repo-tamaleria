document.addEventListener('DOMContentLoaded', () => {
    ...
    if (menuToggle && nav && menuItems) {
        menuToggle.addEventListener('click', (event) => {
            ...
            menuItems.style.display = nav.classList.contains('active') ? 'flex' : 'none';
        });
    ...
    if (searchInput && articulos.length > 0) {
        searchInput.addEventListener('input', (e) => {
            ...
            articulos.forEach(articulo => {
                ...
                articulo.style.display = (titulo.includes(searchTerm) || contenido.includes(searchTerm)) ? 'block' : 'none';
            });
        });
    } else {
